...
//!
//! 1. Match struct is exactly 12 bytes (repr(C), GPU-compatible)
//! 2. Match ordering is deterministic (sort by start, then pattern_id, then end)
//! 3. Match from_parts roundtrip for all u32 extremes (0, 1, u32::MAX-1, u32::MAX)
//! 4. Matcher trait can be implemented by a trivial struct
//! 5. BlockMatcher trait produces correct block-level results
#![allow(
    clippy::unwrap_used,
    clippy::expect_used,
    clippy::panic,
...
/// Verify Match is exactly 12 bytes for GPU buffer compatibility
#[test]
fn match_is_exactly_12_bytes() {
    assert_eq!(
        std::mem::size_of::<Match>(),
        12,
        "Match struct must be exactly 12 bytes for GPU compatibility"
    );
}
...
/// Verify GpuMatch is also 12 bytes
#[test]
fn gpu_match_is_exactly_12_bytes() {
    assert_eq!(
        std::mem::size_of::<GpuMatch>(),
        12,
        "GpuMatch must be exactly 12 bytes for GPU buffer compatibility"
    );
}
...
/// Verify Match ordering is deterministic: start, then pattern_id, then end
#[test]
fn match_ordering_is_deterministic() {
    // Create matches with different combinations
    let m1 = Match::new(1, 10, 20); // pattern_id=1, start=10, end=20
    let m2 = Match::new(2, 10, 20); // same start/end, different pattern_id
    let m3 = Match::new(1, 15, 20); // same pattern_id/end, different start
    let m4 = Match::new(1, 10, 25); // same pattern_id/start, different end
...
fn match_sorting_is_stable_and_deterministic() {
    ...
    assert_eq!(matches[0], Match::new(1, 5, 10)); // smallest start
    assert_eq!(matches[1], Match::new(1, 10, 15)); // same start, smallest end
    assert_eq!(matches[2], Match::new(1, 10, 20)); // same start/pattern_id, larger end
    assert_eq!(matches[3], Match::new(2, 10, 25)); // same start, larger pattern_id
    assert_eq!(matches[4], Match::new(3, 30, 40)); // largest start
}
...
fn matcher_trait_can_be_implemented() {
    ...
    assert_eq!(matches[0], Match::new(1, 0, 10));
}

/// Verify Matcher works with generics
#[test]
...
fn block_matcher_produces_correct_results() {
    let matcher = TrivialBlockMatcher {
        max_block: 4096,
        matches: vec![Match::new(0, 0, 10), Match::new(1, 20, 30)],
    };
...
fn match_contains_and_overlaps() {
    let outer = Match::new(0, 10, 50);
    let inner = Match::new(0, 20, 40);
    let adjacent = Match::new(0, 50, 60);
    let separate = Match::new(0, 60, 70);
    let overlapping = Match::new(0, 40, 60);
...
fn match_len_and_is_empty() {
    let empty = Match::new(0, 10, 10);
    let non_empty = Match::new(0, 10, 20);
