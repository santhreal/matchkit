...
## North Star
Be as fundamental and trusted as `std::time::Duration` or `http::StatusCode`: a universal vocabulary type that no Rust developer questions. Target means zero breaking changes, guaranteed GPU-compatible memory layout, and traits so ergonomic that third-party engines naturally implement them.
